...
public class Hangman implements KeyListener
{
    ...
    private int readWordList(String fileName, String[] wordList) throws IOException 
    {
        ...
        while(input.hasNextLine()){
            wordList[wordCount] = input.nextLine();
            wordCount++;
        }
    ...
    private String pickSecretWord(String[] wordList, int wordListSize)
    {
        ...
        return wordList[randomWordPick.nextInt(wordListSize - 1)];
    }
    ...
    private String updateDisplayWord(String displayWord, String secretWord, char guess)
    {
        ...
        char[] charSecretArray = secretWord.toCharArray();
        char[] charDisplayArray = displayWord.toCharArray();
        
        for(int i = 0; i < secretWord.length(); i++){
            if(charSecretArray[i] == guess){
                charDisplayArray[i] = guess;
                intermediateString += guess;
            }
            else{
                intermediateString += charDisplayArray[i];
            }
        }
